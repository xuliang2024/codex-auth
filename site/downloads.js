(() => {
  const DOWNLOAD_OPTIONS = {
    macosArm64: {
      href: "/downloads/codex-auth-desktop-0.2.3-macos-arm64.dmg",
      label: "下载 macOS Apple 芯片版",
      title: "下载 macOS Apple Silicon DMG",
    },
    macosX64: {
      href: "/downloads/codex-auth-desktop-0.2.3-macos-x64.dmg",
    ...
    windowsX64: {
      href: "/downloads/codex-auth-desktop-0.2.3-win-x64.exe",
      label: "下载 Windows 应用",
      title: "下载 Windows x64 安装包",
    },
    windowsArm64: {
      href: "/downloads/codex-auth-desktop-0.2.3-win-arm64.exe",
  ...
  function pickDownloadOption(platform, architecture, userAgent) {
    ...
    if (/mac|darwin/i.test(platformText)) {
      if (/arm|aarch64/i.test(architectureText)) {
        return DOWNLOAD_OPTIONS.macosArm64;
      }

      if (/x86|x64|amd64|intel/i.test(architectureText)) {
        return DOWNLOAD_OPTIONS.macosX64;
      }

      return DOWNLOAD_OPTIONS.macosAll;
    }
